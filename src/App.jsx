                   ...
                   {/* <header className="bg-white/70 backdrop-blur-lg shadow-sm p-4 text-gray-800 flex items-center border-b border-gray-200/80 flex-shrink-0">
                    ...
                    <main ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-gradient-to-br from-gray-50 to-gray-100">
                        {messageHistory.map((message, idx) => (
                            <div key={idx} className={`flex items-end gap-4 animate-fadeInUp ${message.author === 'user' ? 'justify-end' : 'justify-start'}`}>
                                {/* {message.author === 'bot' && (
                                    // <div className="w-11 h-11 bg-gradient-to-br from-[#03335B] to-[#1279BE] rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border-2 border-[#03335B]">
                                    //     <img src={botLogo} alt="Bot Logo" className="w-9 h-9 rounded-full object-cover" />
                                    // </div>
                                    <></>
                                )} */}
                                <div className={`max-w-xs md:max-w-md lg:max-w-3xl px-6 py-4 rounded-2xl shadow-md ${
                                    message.author === 'user' 
                                        ? 'bg-[#d9ebf7] text-black rounded-br-none' 
                                        : 'bg-white text-gray-800 rounded-bl-none border border-gray-200/80'
                                }`}>
                                    <MessageRenderer message={message.data} />
                    ...
                    <footer className="bg-white/70 backdrop-blur-lg border-t border-gray-200/80 p-4 flex-shrink-0">
                        <div className="max-w-4xl mx-auto flex items-center gap-4">
                            <input
                                ...
                                onKeyPress={handleKeyPress}
                                placeholder="Ask me anything..."
                                className="w-full px-6 py-4 text-base border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#03335B] transition-all duration-300 shadow-sm"
                                disabled={!isConnected}
                            />
                            <button
                                onClick={handleSendClick}
                                disabled={!isConnected || !inputValue.trim()}
                                className="bg-[#03335B] hover:bg-[#022644] disabled:bg-gray-300 disabled:cursor-not-allowed text-white p-4 rounded-full flex-shrink-0 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#03335B]"
                            >
                                <SendIcon />
                            </button>
                        </div>
